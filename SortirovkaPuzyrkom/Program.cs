        ...
        static void Main(string[] args)
        {
            ...
            do
            {
                ...
                if (int.TryParse(length_str, out length))
                    break;
                Console.WriteLine("Неверный формат целого числа");
            }
            while (true);
        ...
        static void SortArray(int[] data)
        {
            ...
            while (work)
            {
                ...
                for (var i = 0; i < data.Length - 1; i++)
                    if (data[i] < data[i + 1]) // можно менять знак больше и меньше и он будет сортировать по-разному
                    {
                        work = true;
                        var temp = data[i];
                        data[i] = data[i + 1];
